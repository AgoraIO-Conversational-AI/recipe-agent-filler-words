...
| `src/filler_config.py`            | `build_filler_words()` / `build_farewell()` — pure builder functions.       |
| `scripts/run_fake_server.py`      | Boots `server.app` with a `FakeAgent` for the local FastAPI smoke test.     |
| `tests/test_agent_construction.py`| Builds the real `AgoraAgent`, fakes the SDK session, asserts result shape.  |
| `tests/test_filler_config.py`     | Asserts `build_filler_words()` and `build_farewell()` payloads.             |
...
- `POST /startAgent` — start the filler-words agent session.
