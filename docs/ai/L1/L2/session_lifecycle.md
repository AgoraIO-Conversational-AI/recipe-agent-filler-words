...
3. **Start agent** — `startAgent(channelName, rtcUid, userUid)` → `POST /api/startAgent`. Backend validates credentials, builds the cascading pipeline, starts the async session, and returns `agent_id`.
4. **Converse** — user audio flows through Deepgram STT → OpenAI LLM → MiniMax TTS; filler phrases play during LLM latency; RTM delivers transcript + metrics.
5. **Stop** — `stopAgent(agentId)` → `POST /api/stopAgent`. The backend triggers `farewell_config` graceful exit; the client also releases RTC/RTM media on end-call.
...
- **Backend owns:** token minting, pipeline build (STT/LLM/TTS), filler/farewell config, session start/stop.
