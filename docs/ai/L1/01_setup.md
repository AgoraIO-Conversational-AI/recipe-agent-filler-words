...
> Install dependencies, configure env, and run the filler-words recipe locally. This recipe is **zero-key** — only Agora credentials are required; `OPENAI_API_KEY` is optional.
...
| `OPENAI_API_KEY`        |          | —                               | Optional BYO key; Agora manages the OpenAI key by default (keyless) |
...
# OPENAI_API_KEY is optional — skip unless your account requires it
```
...
Open <http://localhost:3000> → **Start Conversation** → speak; listen for filler phrases during LLM latency. Backend API docs at <http://localhost:8000/docs>.
